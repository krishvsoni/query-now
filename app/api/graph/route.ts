...
import { searchEntities, getEntityRelationships, debugGraphData } from '@/lib/neo4j';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const mode = searchParams.get('mode') || 'central';
    ...
    if (clearCache) {
      console.log('[Graph API] Clearing cache for user', user.id);
      const cacheKey = `central-kg:${user.id}`;
      await pipeline.getCachedGraphData(user.id, cacheKey);
    }

    await debugGraphData(user.id);
